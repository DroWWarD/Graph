...
public class HelloController {
    ...
    private double[] yArray = new double[1001];

    @FXML
    void initialize() {
        draw.setOnAction(actionEvent -> {
            String expression = getData.getText();
            double[] y = new double[21];
            XYChart.Series series = new XYChart.Series();
            series.setName(getData.getText());
            for (int i = -500, j = 0; i <= 500; i++, j++) {
                recurse(expression.replaceAll("x", String.valueOf(i)), 0, j);
                System.out.println(yArray[j]);
                series.getData().add(new XYChart.Data(String.valueOf(i), yArray[j]));
            }
    ...
    public void recurse(final String expression, int countOperation, int y) {
...
//------------------------- СКОБКИ С МАКСИМАЛЬНЫМ ВЛОЖЕНИЕ ПОСЧИТАНЫ, ДЕЛАЕМ РЕКУРСИЮ, ЗАМЕНЯЯ ИХ--------------------------------
        ...
        if (expression.contains("(")) {
            ...
            recurse(newExpression, newCount, y);
        } else {
            ...
            yArray[y] = Double.parseDouble(tempResult);
